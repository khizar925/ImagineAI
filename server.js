...
// Get user's image history metadata only (FAST)
app.get("/api/history-metadata", authenticateToken, (req, res) => {
  ...
  db.get(
    ...
    (err, countResult) => {
      ...
      // Get metadata only (no image_data)
      db.all(
        "SELECT id, prompt, created_at FROM generated_images WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?",
...
// Get individual image data (on-demand)
app.get("/api/image/:imageId", authenticateToken, (req, res) => {
  const imageId = req.params.imageId;

  db.get(
    "SELECT image_data FROM generated_images WHERE id = ? AND user_id = ?",
...
app.get("/api/download/:imageId", authenticateToken, (req, res) => {
  ...
  db.get(
    ...
    (err, row) => {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="generated-image-${imageId}.png"`
      );
      res.send(buffer);
    }
  );
